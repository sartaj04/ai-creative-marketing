"""Google Gemini LLM provider using Google Gen AI SDK."""
import asyncio
import json
import logging
...
# Thread pool executor for running blocking SDK calls
...
class GeminiProvider(LLMProvider):
    ...
    def __init__(self):
        """Initialize Gemini provider with Google Gen AI credentials."""
        self._initialized = False
        self._client = None
        self._initialize_genai()

    def _initialize_genai(self):
        """Initialize Google Gen AI with service account credentials."""
        try:
            ...
            logger.info("Google Gen AI Gemini initialized successfully")
    ...
    async def generate_json(
        ...
        """Generate structured JSON response using native JSON mode."""
        if not self._initialized or not self._client:
            raise RuntimeError("Gemini provider not initialized. Check GCP credentials.")
        ...
        try:
            ...
            logger.debug(f"Raw LLM response length: {len(response.text)}")
            logger.debug(f"Raw response preview: {response.text[:200]}...")

            # Parse JSON response (should be valid JSON from native mode)
            try:
    ...
    async def embed(self, text: str) -> list[float]:
        ...
        try:
            ...
            if response and hasattr(response, 'embeddings') and response.embeddings:
                ...
                # Try direct access if it's a list/array
