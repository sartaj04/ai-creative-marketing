...
import { getFieldByKey, isFieldEmpty, FieldSchema } from '@/lib/schemas/identity-schema';
                                    ...
                                    <div className={cn(
                                        ...
                                        {region.id === 'core' && (
                                            <>
                                                <div className="text-xl font-bold text-slate-900 leading-tight mb-1 font-serif">
                                                    {region.data.role || "Undefined Role"}
                                                </div>
                                        ...
                                        {region.id === 'content' && (
                                            <div className="flex flex-wrap gap-1 justify-center">
                                                ...
                                                {(!region.data.pillars?.length) && <span className="text-slate-400 text-xs italic">No pillars defined</span>}
                                            </div>
                                        )}
                                        ...
                                        {region.id === 'strategy' && (
                                            <div className="text-xs text-emerald-800 leading-relaxed italic line-clamp-3">
                                                &ldquo;{region.data.audience || "No target audience defined."}&rdquo;
                                            </div>
                                        )}

                                        {region.id === 'character' && (
                                            <div className="flex flex-wrap gap-1 justify-center max-w-[180px]">
                                                {(region.data.beliefs || []).slice(0, 2).map((b: string, i: number) => (
                                                    <span key={i} className="px-2 py-0.5 border border-amber-200 text-amber-800 text-[10px] rounded hover:bg-amber-100">
                                                        {b.length > 20 ? b.substring(0, 20) + '...' : b}
                                                    </span>
                                                ))}
                                            </div>
                                        )}
                            ...
                            <div className="flex items-center justify-between mb-8">
                                <div>
                                    ...
                                    <h2 className="text-3xl font-bold text-slate-900 font-serif">
                                        {activeRegion.id === 'core' ? activeRegion.data.role : activeRegion.label}
                                    </h2>
                                </div>
                                <button
